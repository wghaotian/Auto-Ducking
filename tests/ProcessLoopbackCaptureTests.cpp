...
int RunProcessLoopbackCaptureTests() {
    ...
    check(auto_ducking::CalculatePcm16Peak(silence, 4) == 0.0F,
          "PCM peak handles silence");

    const std::int16_t halfScale[] = {100, -16384, 1200, 0};
    check(std::fabs(auto_ducking::CalculatePcm16Peak(halfScale, 4) - 0.5F) < 0.0001F,
          "PCM peak handles signed samples");

    const std::int16_t fullScale[] = {-32768, 32767};
    check(auto_ducking::CalculatePcm16Peak(fullScale, 2) == 1.0F,
          "PCM peak handles negative full scale");
    check(auto_ducking::CalculatePcm16Peak(nullptr, 10) == 0.0F,
